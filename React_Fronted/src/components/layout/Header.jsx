...
function Header() {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty("--header-height", hideHeader ? "0px" : "5.5rem");
  }, [hideHeader]);
            ...
            <div className="hidden md:flex items-center gap-4">
              {visibleLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    getNavClass({ isActive: computeIsActive(link.path) || isActive })
                  }
                >
                  {link.name}
                </NavLink>
              ...
              <div className="mt-4 flex flex-col gap-3">
                ...
                {!isAuthenticated && (
                  <button
                    onClick={() => {
                      setMenuOpen(false);
                      navigate("/register", { state: { from: "/plans" } });
                    }}
                    className="w-full py-3 rounded-xl
