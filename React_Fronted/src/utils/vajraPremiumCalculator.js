...
export const VAJRA_FEATURE_LABELS = {
  ...
  'hosp_mandatory': 'Hospitalisation Cover',
...
export const VAJRA_RIDER_LABELS = {
  ...
  'ped_wait': 'PED Wait Reduction',
  'ped_wait_2y': 'PED Wait Reduction (4yr→2yr)',
  'ped_wait_1y': 'PED Wait Reduction (4yr→1yr)',
...
export const calculateVajraPremium = ({
  ...
  if (VAJRA_FEATURE_COSTS_BASE[preHospKey]) {
    ...
    breakdown.items.push({
      type: 'feature',
      label: VAJRA_FEATURE_LABELS[preHospKey] || preHospKey,
      cost
    });
  }
  ...
  if (VAJRA_FEATURE_COSTS_BASE[postHospKey]) {
    ...
    breakdown.items.push({
      type: 'feature',
      label: VAJRA_FEATURE_LABELS[postHospKey] || postHospKey,
      cost
    });
  }
  ...
  features.forEach(feat => {
    ...
    if (cost > 0) {
      ...
      breakdown.items.push({
        type: 'feature',
        label: VAJRA_FEATURE_LABELS[featId] || featId,
        cost
      });
    }
  });
  ...
  riders.forEach(rider => {
    ...
    if (cost > 0) {
      ...
      breakdown.items.push({
        type: 'rider',
        label: VAJRA_RIDER_LABELS[riderId] || riderId,
        cost
      });
    }
  });
